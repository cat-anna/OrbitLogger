...
StreamReader::StreamReader(FILE *Stream, LogChannel Channel) {
    ...
    fdOut = _dup(_fileno(Stream));
    fflush(Stream);
    if (_dup2(fdOutPipe[WRITE_FD], _fileno(Stream)) != 0) {
...
StreamReader::~StreamReader() { m_ThreadCanRun = false; }

void StreamReader::ThreadMain() {
    ThreadInfo::SetName("STRD");
    const LogLineSourceInfo SrcInfo(m_Channel, __FILE__, __FUNCTION__,
                                    __LINE__);
        ...
        for (int i = 0; i < got; ++i) {
            ...
            case '\n':
                ...
                if (::OrbitLogger::LogCollector::IsChannelEnabled(&SrcInfo))
                    ::OrbitLogger::LogCollector::PushLine(&SrcInfo,
                                                          buffer.c_str());
                buffer.clear();
            case '\r':
                continue;
